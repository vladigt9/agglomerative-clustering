...
private:
    ...
    std::vector<LinkageRow> clustering()
    {
        ...
        std::vector<double> condensedArray((n * (n - 1) / 2), std::numeric_limits<double>::max());
        
        // Initialize map used to deduce indices of clusters
        std::vector<std::pair<int, int>> condensedArrayMap (n * (n - 1) / 2);

        // Initialize map used to deduce intial cluster from index and current cluster from value
        std::vector<int> id_map (n);

        // Get intial distances in condensed array
        ...
        for (std::size_t i = 0; i < (n-1); ++i)
        {
            ...
            int validDistances = (validClusters * (validClusters - 1)) / 2 + validClusters;
            ...
            for (std::size_t k = 0; k < remainingIndices.size()-1; ++k)
            {
                ...
                condensedArray[validDistances+k] = Linkage(cluster1, cluster2);
            }
...
int main()
{
    ...
    Clustering<double> cluster {X, "single", "euclidean"};
